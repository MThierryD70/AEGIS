...
class EnvironmentDetector:
    ...
    def detect(self) -> EnvReport:
        ...
        report.can_buid = gpp_ok and cmake_ok and openssl_ok
    ...
    def _find_openssl_lib(self) -> str:
        if platform.system() == "windows":
            # Chercher libssl.lib ou libssl.a
            for base in self.OPENSSL_SEARCH_PATHS_WIN:
                for subpath in [
                    "lib/VC/x64/MD",
                    "lib/VC/x64/MT",
    ...
    def _find_mingw_bin(self) -> str:
        if platform.system() != "windows":
            return ""
        for path in self.MINGW_SEARCH_WIN:
            if Path(path).exists():
                return path

        # Chercher g++ dans PATH et remonte vers bin/
        gpp = shutil.which("g++")
        if gpp:
